...
import { IncomingPacketIDs, IncomingPacketTypes, OutgoingPacketIDs } from './Packets';
import { FriendRequest, OfflineUser, OnlineUser, User } from './Types';

export async function handle<T extends keyof IncomingPacketTypes>(client: Client, id: T, data: any) {
  switch (id) {
    ...
    case IncomingPacketIDs.JoinServer:
      if (client.friends.get(data.uuid).online) (client.friends.get(data.uuid) as OnlineUser).server = data.server || 'In Menus';
      else {
    ...
    case IncomingPacketIDs.PlayerInfo:
      if (data.uuid == client.uuidWithDashes) client.user = data;
        ...
        if (client.friends.get(data.uuid).online) (client.friends.get(data.uuid) as OnlineUser).lcUser = data;
    ...
    case IncomingPacketIDs.FriendRequest:
      var req = {
        uuid: data.uuid,
        username: data.username || (await client.userManager.fetchUUID(data.uuid)).username,
        outgoing: false,
      } as FriendRequest;
    ...
    case IncomingPacketIDs.FriendResponse:
      ...
      client.emit('friendRequestHandled', client.friendRequests.get(data.uuid), data.accepted);
    ...
    case IncomingPacketIDs.PendingRequests:
      data.bulk.forEach(async request => {
        const req = {
          uuid: request.uuid,
          username: request.username || (await client.userManager.fetchUUID(request.uuid)).username,
          outgoing: false,
        } as FriendRequest;
    ...
    case IncomingPacketIDs.FriendMessage:
      if (client.friends.get(data.uuid)?.online) {
        ...
        client.emit('friendMessage', client.friends.get(data.uuid), data.message || '');
      }
      break;
    ...
    case IncomingPacketIDs.PlayEmote:
      if (client.users.get(data.uuid)?.online) client.emit('playEmote', client.users.get(data.uuid) as OnlineUser, data.id, data.metadata);
      break;
    default:
