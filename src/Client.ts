...
export class Client extends (EventEmitter as new () => TypedEmitter<ClientEvents>) {
	...
	/** The Client Mojang User Manager */
	 ...
	 */
	private send<T extends keyof OutgoingPacketTypes>(
		id: T,
		data: OutgoingPacketTypes[T]
	): Promise<void> {
		return new Promise((res, rej) =>
			this.socket.send(
				(writePacket(id, data) as Packet).buf.buffer,
				(err) => (err ? rej(err) : res())
			)
		);
	}
	 ...
	 * @param data Packet Data
	 */
	public async sendPersistent<T extends keyof OutgoingPacketTypes>(
		id: T,
		data: OutgoingPacketTypes[T]
	) {
	 ...
	 * @param data Packet Data
	 */
	public async sendImpersistent<T extends keyof OutgoingPacketTypes>(
		id: T,
		data: OutgoingPacketTypes[T]
	) {
		if (this.socket?.readyState === WebSocket.OPEN)
					...
					this.friends.set(
						...
						this.users.get(uuid) ||
							({
								uuid,
								username:
									this.friendRequests.get(uuid).username,
	...
	public async toggleFriendRequests(enabled: boolean) {
		...
		if (
			!(await this.sendPersistent(
				OutgoingPacketIDs.ToggleFriendRequests,
				{
					status: isEnabled,
				}
			))
		...
		await Promise.all(
			uuids.map(async (uuid) => {
				if (users[uuids.indexOf(uuid)]?.uuid != uuid) {
					const user = {
						uuid,
						username: (await this.userManager.fetchUUID(uuid))
							.username,
						online: false,
						offlineSince: Date.now(),
					} as OfflineUser;
